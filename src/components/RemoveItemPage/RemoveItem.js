...
const RemoveItem=(props)=>{
    ...
    const senData=async()=>{
        const Url="http://localhost:80/RemoveItem";
        const Body=JSON.stringify({
            itemName,
            itemType,
            itemColor,
        ...
        <div id="AddItemContainer">
            ...
            <form onSubmit={formSubmitHandler}>
                <div className="inputcontainer">
                    <h4>Item Name : </h4>
                    <input value={itemName} onChange={itemNameHandler} name="Name" type="text" list="itemnamelist"></input>
                    {itemsName.length!==0&&<datalist id="itemnamelist">{itemsName.map((value)=>{return <option>{value.itemname}</option>})}</datalist>}
                </div>
                <div className="inputcontainer">
                    <h4>Item Type : </h4>
                    <input value={itemType} onChange={itemTypeHandler} name="Type" type="text" list="itemtypelist"></input>
                    {itemsType.length!==0&&<datalist id="itemtypelist">{itemsType.map((value)=>{return <option>{value.itemtype}</option>})}</datalist>}
                </div>
                <div className="inputcontainer">
                    <h4>Item Color : </h4>
                    <input value={itemColor} onChange={itemColorHandler} name="Color" type="text" list="itemcolorlist"></input>
                    {itemsColor.length!==0&&<datalist id="itemcolorlist">{itemsColor.map((value)=>{return <option>{value.itemcolour}</option>})}</datalist>}
                </div>
                <div className="inputcontainer">
                    <h4>Item Quantity : </h4>
                    <input value={itemQuantity} onChange={itemQuantityHandler} name="Quantity" type="number"></input>
                </div>
